...
/*----------------------------------------------------
  ...
  it is updated for stata 13 or up, and hlm 7
----------------------------------------------------*/

capture program drop hlm3_s
program define hlm3_s
version 13
...
*-------------
if "`run'" ~= "" {
	hlmrun_s `filename'.hlm, hlm3_s mdm($mdmfile) noview `store' `robust'
	view `outfile'.txt
}
