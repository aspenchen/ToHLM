...
----------------------------------------------------*/
capture program drop hlm2_s
program define hlm2_s
version 13
...
if "$mdmfile" == "" {
	...
	error 601 // "file ______ not found"
}
...
*-------------
if "`run'" ~= "" {
	hlmrun_s `filename'.hlm, hlm2s mdm($mdmfile) noview `store' `robust'
	view `"`outfile'.txt"'
}
