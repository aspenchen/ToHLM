*** Reading notes on reardon's hlm package
...
if `"`filename'"' == "" global mdmfile ""
	...
	if `col' == 1 {
		...
		confirm file `"`c(pwd)'`c(dirsep)'`path'_mdmvars.dta"' // where did the c(pwd)/c(dirsep) came from?
